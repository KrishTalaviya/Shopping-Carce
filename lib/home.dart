...
import 'package:shopingapp/carousel.dart';
import 'package:shopingapp/navdrawer.dart';
import 'package:shopingapp/profilepage.dart';
import 'dart:async';
import 'dart:convert';

import 'package:shopingapp/shoppingcart.dart';
import 'package:shopingapp/wishlist.dart';
...
class _HomeState extends State<Home> {
  int? c;
  int n = 0;
  Widget getChoice(c, n) {
    if (c == 1) {
      return FutureBuilder<List<Shoes>>(
        future: futureAlbum,
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            return Text(snapshot.data![n].name!);
      ...
      return FutureBuilder<List<Shoes>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            return Image.network(snapshot.data![n].image!);
          } else if (snapshot.hasError) {
            return Text('${snapshot.error}');
          }
      ...
      return FutureBuilder<List<Shoes>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            return Text(snapshot.data![n].price!);
          } else if (snapshot.hasError) {
            return Text('${snapshot.error}');
          }
  ...
  late Future<List<Shoes>> futureAlbum;
                              ...
                              Container(
                                ...
                                child: Card(
                                  child: Column(
                                    ...
                                    children: [
                                      Container(
                                          height: 100, child: getChoice(2, 1)),
                                      Container(
                              ...
                              Container(
                                ...
                                child: Card(
                                  child: Column(
                                    ...
                                    children: [
                                      Container(
                                          height: 100, child: getChoice(2, 4)),
                                      Container(
                                        margin: EdgeInsets.all(5),
                                        child: getChoice(1, 4),
                                      ),
                              ...
                              Container(
                                ...
                                child: Card(
                                  child: Column(
                                    ...
                                    children: [
                                      Container(
                                          height: 100, child: getChoice(2, 6)),
                                      Container(
                                        margin: EdgeInsets.all(5),
                                        child: getChoice(1, 6),
                                      ),
...
Future<List<Shoes>> fetchAlbum() async {
  final response = await http
      .get(Uri.parse('https://63fa074d897af748dcc7e782.mockapi.io/shoes'));
  ...
  if (response.statusCode == 200) {
    ...
    for (int i = 0; i < data.length; i++) {
      listUser.add(Shoes.fromJson(data[i]));
    }
    return listUser;
  } else {
